...
def gerar_xml_reinf(dados, per_apur):
    ...
    if float(dados["vlrBaseAgreg"].replace(",", ".")) <= 0 or float(dados["vlrAgreg"].replace(",", ".")) <= 0:
        print(f"Evento sem retenção, não gerado para o CNPJ: {dados['nrInsc']}!")
...
def Agregar(eventos):
    ...
    for nrInscEstab, lista_eventos in eventos_agrupados.items():
        agregados = {}

        for evento in lista_eventos:
            nat_rend = evento["natRend"]

            if nat_rend not in agregados:
                agregados[nat_rend] = {
                ...
                # Se natRend for igual, soma valores
                agregados[nat_rend]["vlrBaseAgreg"] += float(evento["vlrBaseAgreg"].replace(",", "."))
                agregados[nat_rend]["vlrAgreg"] += float(evento["vlrAgreg"].replace(",", "."))
                agregados[nat_rend]["vlrBruto"] += float(evento["vlrBruto"].replace(",", "."))
                agregados[nat_rend]["observ"] = "RECOLHIMENTO DE TRIBUTOS FEDERAIS SOBRE 2 OU MAIS NF-Es, EM ATENDIMENTO A IN/RFB 1234/2012."

        eventos_processados.extend(agregados.values())
